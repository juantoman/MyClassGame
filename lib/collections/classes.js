...
Meteor.methods({
  classInsert: function(classe) {
    ...
    //console.log(classId);
    Meteor.call('teacherInClass',classId)
    return classId;
  ...
  classDelete: function(classId) {
    ...
    var Id = chalMissions.remove({ classId: classId });
    var Id = challengesXP.remove({ classId: classId });
    var Id = chatClass.remove({ classId: classId });
    var Id = chatStudentTeacher.remove({ classId: classId });
    var Id = chatTeachers.remove({ classId: classId });
    var Id = diary.remove({ classId: classId });
    var Id = notebook.remove({ classId: classId });
    var Id = notebookWork.remove({ classId: classId });
    var Id = notifications.remove({ classId: classId });
    var Id = questions.remove({ classId: classId });
    var Id = quizzes.remove({ classId: classId });
    //var Id = tasks.remove({ classId: classId });
    var Id = villains.remove({ classId: classId });
  ...
  levelXPRatioUpdate: function(classId,levelXPRatio) {
    var Id = classes.update({ _id: classId }, { $set: {levelXPRatio: levelXPRatio} });
  },
  ...
  mapUpdate: function(classId,mapImg) {
    var Id = classes.update({ _id: classId }, { $set: {mapImg: mapImg} });
  },
  ...
  medalsVisibleChange: function(classId,medalsVisible) {
    var Id = classes.update({ _id: classId }, { $set: {medalsVisible: medalsVisible} });
  },
  ...
  classDuplicate: function(c,cId) {
    ...
    images.find({'classId': cId}).forEach(function(item){
      ...
      behaviours.update({'classId': cnId, 'behaviourImage': ida}, {$set: {'behaviourImage': idn}}, { multi: true} );
    });
  ...
  resetClass: function(classId) {
    ...
    challengesXP.remove({ classId: classId });
    behavioursLog.remove({ classId: classId });
  },
  ...
  addChromeType: function(classId,chromeTypeDesc) {
    var chromeType =  {
      _id: new Meteor.Collection.ObjectID()._str,
      chromeTypeDesc: chromeTypeDesc,
      chromeTypeColor: '#268BD2'
    };
    var Id =  classes.update({ _id: classId }, { $push: {chromeTypes: chromeType}} );
  },
  delChromeType: function(classId,chromeTypeId) {
    var Id =  classes.update({ _id: classId }, { $pull: {chromeTypes: {_id: chromeTypeId}}} );
  },
  updateChromeTypeColor: function(classId, chromeId, chromeTypeColor) {
    var Id =  classes.update({ '_id': classId, 'chromeTypes._id': chromeId }, { $set: {'chromeTypes.$.chromeTypeColor': chromeTypeColor}} );
  },
  updateChromeTypeDesc: function(classId, chromeId, chromeTypeDesc) {
    var Id =  classes.update({ '_id': classId, 'chromeTypes._id': chromeId }, { $set: {'chromeTypes.$.chromeTypeDesc': chromeTypeDesc}} );
  },
  ...
  CoinsRelChange: function(classId,CoinsRel) {
    var Id = classes.update({ _id: classId }, { $set: {'CoinsRel': CoinsRel} });
  },
  XPsRelChange: function(classId,XPsRel) {
    var Id = classes.update({ _id: classId }, { $set: {'XPsRel': XPsRel} });
  },
  classAlive: function(classId) {
    classes.update({ _id:classId }, { $set: {'alive': true} });
  },
  deleteClassesNotAlive: function(classId) {
    //n=0;
    //classes.find({'alive': { $exists: false }},{limit:100}).forEach( function(c) {
    classes.find({'alive': false },{limit:100}).forEach( function(c) {
      // n++;
      // console.log(n);
      // if (n>=100) {
      //   console.log("Fin");
      // }
      Meteor.call('classDelete',c._id);
    })
  },
  deleteDemoClasses: function(classId) {
    classes.find({'teacherId': { '$regex': '^qTDQNMMRA'} },{limit:100}).forEach( function(c) {
      Meteor.call('classDelete',c._id);
    })
  },
  aliveFalseClasses: function(classId) {
    classes.update({}, { $set: {'alive': false} } , { multi: true} );
  },
  envelopePrice: function(classId,envelopePrice) {
    classes.update({ _id:classId }, { $set: {'envelopePrice': envelopePrice} });
  },
  stickersEnvelope: function(classId,stickersEnvelope) {
    classes.update({ _id:classId }, { $set: {'stickersEnvelope': stickersEnvelope} });
  },
  visibleClass: function(classId,visible) {
    classes.update({ _id:classId }, { $set: {'visibleClass': visible} });
  },
  incNCopies: function(classId,) {
    classes.update({ '_id': classId },{ $inc: { 'ncopies': 1 } });
  },
  myXPName: function(classId,XPName) {
    var Id = classes.update({ _id: classId }, { $set: {'myClassWords.myXPName': XPName} });
  },
  myHPName: function(classId,HPName) {
    var Id = classes.update({ _id: classId }, { $set: {'myClassWords.myHPName': HPName} });
  },
  myMoneyName: function(classId,MoneyName) {
    var Id = classes.update({ _id: classId }, { $set: {'myClassWords.myMoneyName': MoneyName} });
  }
});
