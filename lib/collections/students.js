...
Meteor.methods({
  ...
  studentXP: function(studentId,xp) {
    ...
    if (classes.findOne({ '_id': classId}).CoinXP ) {
      CoinsRel=parseInt(classes.findOne({ '_id': classId}).CoinsRel);
      XPsRel=parseInt(classes.findOne({ '_id': classId}).XPsRel);
  ...
  incCoins: function(studentId,price) {
    var Id =  students.update({ _id: studentId }, { $inc: {coins: parseInt(price)} });
  },
