...
Meteor.methods({
  ...
  studentXP: function(studentId,xp) {
    ...
    switch (parseInt(c.evaluation))  {
      case 1:
        var Id =  students.update({ _id: studentId }, { $inc: {XP1: xp} });
        break;
      case 2:
        var Id =  students.update({ _id: studentId }, { $inc: {XP2: xp} });
        break;
      case 3:
        var Id =  students.update({ _id: studentId }, { $inc: {XP3: xp} });
        break;
    }
    if ( c.CoinXP ) {
      ...
      if (isNaN(rel)) {rel=1;}
      coins=parseFloat(xp*rel)+parseFloat(s.coins);
      //coins=parseInt(xp*rel*100)/100+s.coins;
      var Id =  students.update({ '_id': studentId }, { $set: {coins: coins.toFixed(2)} });
    }
  ...
  studentXPapi: function(studentId,xp) {
    xp = isNaN(xp) ? parseInt(0): parseInt(xp);
    student="^"+studentId;
    var Id =  students.update({ '_id': {$regex: student} }, { $inc: {XP: xp} });
  },
  ...
  usingCard: function(studentId,price) {
    ...
    coins=parseFloat(coins)-parseFloat(price);
    var Id =  students.update({ _id: studentId }, { $set: {coins: coins.toFixed(2)} });
  },
  ...
  incCoins: function(studentId,price) {
    coins =  students.findOne({ _id: studentId }).coins;
    price = isNaN(price) ? 0: price;
    coins=parseFloat(price)+parseFloat(coins);
    var Id =  students.update({ _id: studentId }, { $set: {coins: coins.toFixed(2)} });
  },
