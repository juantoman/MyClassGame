...
Meteor.methods({
  ...
  chalMissionUpdate: function(chalId,chalChanges) {
    var Id = chalMissions.update({ _id: chalId }, { $set: chalChanges });
  },
  ...
  taskVisibleToggle: function(taskId) {
    v=chalMissions.findOne({ _id: taskId }).visible;
    var Id = chalMissions.update({ _id: taskId }, { $set: {visible: !v } });
  }
});
