...
Meteor.methods({
  ...
  missionImageUpdate: function(missionId,imageId) {
    var Id = challenges.update({ _id: missionId }, { $set: {missionImg: imageId } });
  },
  missionVisibleToggle: function(missionId) {
    v=challenges.findOne({ _id: missionId }).missionVisible;
    var Id = challenges.update({ _id: missionId }, { $set: {missionVisible: !v } });
  }
